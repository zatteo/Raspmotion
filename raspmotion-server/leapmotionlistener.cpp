...
void LeapMotionListener::onFrame(const Controller &controller){
    ...
    //  Up - Down
        if (leftPitch > -0.6 && leftPitch < -0.2) { //down
                leftPitch += 0.2;
                percent = floor((leftPitch * 100) / -0.4);
                tmpPitch = (int)percent/10;
        }
        else if (leftPitch > 0.2 && leftPitch < 0.6){ //up
        ...
        // Left - right
        if (leftRoll > -0.6 && leftRoll < -0.2) { //right
        ...
        else if (leftRoll > 0.2 && leftRoll < 0.6){ //left
            tmpIsLoopingRight = false;
            isLoopingRight = false;
        ...
        // Speed up - Speed down
       if (leftPalm > 20. && leftPalm < 65.){  //speed down
           leftPalm -= 20.;
           percent = floor((leftPalm * 100) / 45.);
           tmpSpeed = -(int)percent/10;
       }
       else if (leftPalm > -65. && leftPalm < -20.){ //speed up
           leftPalm += 20.;
           percent = floor((leftPalm * 100) / -45.);
           tmpSpeed = (int)percent/10;
       }
...
void LeapMotionListener::update() {
    ...
    if(tmpIsLoopingUp && !isLoopingUp){
        ...
        qDebug() << " : looping up";
        server->sendMessage(LOOP_VERTICAL);
        return;
    }
